...
public class TestManualScheduling extends AbstractNodeMain {
	...
	private void setupConstraintNetwork() {
		...
		//Decide that turtlebot1 stays parked for at least 10 second
		AllenIntervalConstraint releaseMoveRobot1 = new AllenIntervalConstraint(AllenIntervalConstraint.Type.Release, new Bounds(ORIGIN+10000,APSPSolver.INF));
		...
		//Decide that turtlebot2 stays parked for at least 10 second
		AllenIntervalConstraint releaseMoveRobot2 = new AllenIntervalConstraint(AllenIntervalConstraint.Type.Release, new Bounds(ORIGIN+10000,APSPSolver.INF));
...
//		//Decide that turtlebot1 should start moving at ORIGIN+25 seconds
...
//		trajectoryEnvelopeSolver.addConstraint(releaseMoveRobot1);
//
//		//Decide that turtlebot2 should start moving at ORIGIN+40 seconds
...
//		trajectoryEnvelopeSolver.addConstraint(releaseMoveRobot2);
		...
		for (DE9IMRelation.Type t : DE9IMRelation.getRelations(trajectoryEnvelopeRobot1.getEnvelopeVariable(), trajectoryEnvelopeRobot2.getEnvelopeVariable())) {
			System.out.println("Possible spatial relation: " + t);
		}
		
		//Print the temporal domains
		long earliestStrartTimeRobot1 = trajectoryEnvelopeRobot1.getTemporalVariable().getEST();
		long earliestEndTimeRobot1 = trajectoryEnvelopeRobot1.getTemporalVariable().getEET();
		long earliestStrartTimeRobot2 = trajectoryEnvelopeRobot2.getTemporalVariable().getEST();
		long earliestEndTimeRobot2 = trajectoryEnvelopeRobot2.getTemporalVariable().getEET();
		
		System.out.println("(EST,EET) Robot 1: (" + earliestStrartTimeRobot1 + "," + earliestEndTimeRobot1 + ")");
		System.out.println("(EST,EET) Robot 2: (" + earliestStrartTimeRobot2 + "," + earliestEndTimeRobot2 + ")");
